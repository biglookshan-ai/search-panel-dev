...
const I18N = {
  zh: {
    ...
    'ins.searches': '搜索次数', 'ins.drawerSearches': '弹窗搜索', 'ins.resultsSearches': '结果页搜索', 'ins.zero': '零结果', 'ins.zeroRate': '零结果率', 'ins.clicks': '点击数', 'ins.drawerClicks': '弹窗点击', 'ins.resultsClicks': '结果页点击', 'ins.recClicks': '推荐位点击', 'ins.sessions': '独立访客',
  ...
  en: {
    ...
    'ins.searches': 'Searches', 'ins.drawerSearches': 'Drawer searches', 'ins.resultsSearches': 'Results searches', 'ins.zero': 'Zero-result', 'ins.zeroRate': 'Zero-result rate', 'ins.clicks': 'Clicks', 'ins.drawerClicks': 'Drawer clicks', 'ins.resultsClicks': 'Results clicks', 'ins.recClicks': 'Recommendation clicks', 'ins.sessions': 'Unique visitors',
...
function renderInsOverview(body, s) {
  const tt = s.totals || {};
  const totalSearch = (tt.drawer_searches || 0) + (tt.results_searches || 0);
  const zr = totalSearch ? Math.round((tt.zero / totalSearch) * 100) : 0;   // zero-result rate over all typed search events
  let h = '<div class="ins-stats">';
  h += insStat(t('ins.drawerSearches'), tt.drawer_searches || 0);
  h += insStat(t('ins.resultsSearches'), tt.results_searches || 0);
  h += insStat(t('ins.zeroRate'), zr + '%', (tt.zero || 0) + ' / ' + totalSearch);
...
function renderInsTopSearches(body, s) {
  ...
  const h = rows.map((r) => '<tr><td>' + esc(r.query) + '</td><td class="num">' + (r.drawer_n || 0) + '</td><td class="num">' + (r.results_n || 0) + '</td><td class="num">' + (r.zero ? '<span class="ins-zero">' + r.zero + '</span>' : 0) + '</td></tr>').join('');
  body.innerHTML = insTable('<th>' + t('ins.colQuery') + '</th><th class="num">' + t('ins.colDrawerN') + '</th><th class="num">' + t('ins.colResultsN') + '</th><th class="num">' + t('ins.colZero') + '</th>', h);
}
function renderInsNav(body, s) {
  const rows = s.nav || [];
  if (!rows.length) { body.innerHTML = '<p class="muted">' + t('ins.empty') + '</p>'; return; }
  const h = rows.map((r) => '<tr><td>' + esc(r.query) + '</td><td class="num">' + r.n + '</td></tr>').join('');
  body.innerHTML = insTable('<th>' + t('ins.colQuery') + '</th><th class="num">' + t('ins.colCount') + '</th>', h);
}
